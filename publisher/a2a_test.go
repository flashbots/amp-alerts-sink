...
func TestA2APublisherSendsTemplatedPromptAndStructuredDetails(t *testing.T) {
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path == "/a2a/.well-known/agent-card.json" {
	...
	database.EXPECT().Get(gomock.Any(), alertFiring.IncidentDedupKey()).Return("", nil)
	database.EXPECT().Lock(gomock.Any(), alertFiring.IncidentDedupKey(), timeoutLock).Return(true, nil)
	database.EXPECT().
		Set(gomock.Any(), alertFiring.IncidentDedupKey(), timeoutA2AExpiry, gomock.Any()).
		DoAndReturn(func(_ context.Context, _ string, _ time.Duration, value string) error {
			var stored PublishMetadata
...
func TestA2APublisherReturnsStoredMetadataForDuplicate(t *testing.T) {
	...
	database.EXPECT().
		Get(gomock.Any(), alertFiring.IncidentDedupKey()).
...
func TestA2APublisherSkipsConsecutiveUpdateForSameIncident(t *testing.T) {
	ctrl := gomock.NewController(t)
	database := mock_db.NewMockDB(ctrl)
	p, err := NewA2A(&config.A2A{
		PromptUrl:      "https://agent.example.com",
	...
	// Same incident (labels + startsAt) as alertFiring, but with annotations
	// that changed between notifications (e.g. a templated current value) —
	// alertFiring.MessageDedupKey() would differ from this alert's, but
	// IncidentDedupKey() must match so a2a is only prompted once.
	alertUpdated := &types.AlertmanagerAlert{
		StartsAt: alertFiring.StartsAt,
		Status:   alertFiring.Status,
		Labels:   alertFiring.Labels,
		Annotations: map[string]string{
			"summary": "Notification test, updated value",
		},
	}
	require.Equal(t, alertFiring.IncidentDedupKey(), alertUpdated.IncidentDedupKey())
	require.NotEqual(t, alertFiring.MessageDedupKey(), alertUpdated.MessageDedupKey())
	...
	metadata, err := p.Publish(
		context.Background(), "test-source", alertUpdated, nil,
	)
	require.NoError(t, err)
	assert.Equal(t, "task-123", metadata["task_id"])
	assert.Equal(t, true, metadata["duplicate"])
...
func TestA2APublisherWaitsForSkippedDependency(t *testing.T) {
	...
	metadata, err := p.Publish(
		...
		"test-source",
		alertFiring,
		PublishResults{NameSlack: {Status: ResultStatusAlreadyLocked}},
	)
