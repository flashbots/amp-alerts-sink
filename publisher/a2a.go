...
func newA2aError(err error) a2aError {
	res := a2aError{}
	if errors.As(err, &res) {
		return res
	}
	return a2aError{err: err}
}
...
func (err a2aError) Unwrap() error {
	return err.err
}
...
type a2aPromptData struct {
	...
	Publishers PublishResults           `json:"publishers"`
}
...
func (p *a2aPublisher) Publish(
	...
	previousPublishers PublishResults,
) (PublishMetadata, PublisherError) {
	l := logutils.LoggerFromContext(ctx)

	// don't act on "resolved" status
	if alert.Status == "resolved" {
		return PublishMetadata{"ignore_reason": "incident resolved"}, nil
	}

	for dependencyName, result := range previousPublishers {
		if result.Status == ResultStatusAlreadyLocked {
			...
			return PublishMetadata{"waiting_for": dependencyName}, newA2aError(ErrAlreadyPublishing)
		}
	}
	...
	if isDup {
		...
		if v, err := p.db.Get(ctx, alert.IncidentDedupKey()); err == nil {
			_ = json.Unmarshal([]byte(v), &metadata)
		}
		metadata["duplicate"] = true
		l.Info("Duplicate alert detected", zap.Error(err))
		if err == nil {
			// Already prompted a2a for this incident; nothing went wrong.
			return metadata, nil
		}
		...
		return metadata, newA2aError(err)
	}
	if err != nil {
	...
	if err != nil {
		...
		return nil, newA2aError(fmt.Errorf("failed to send A2A message: %w", err))
	}

	metadata := a2aResponseMetadata(response)
	encodedMetadata, marshalErr := json.Marshal(metadata)
	if marshalErr != nil {
		l.Warn("Failed to encode A2A response metadata for deduplication", zap.Error(marshalErr))
	} else if err := p.db.Set(ctx, alert.IncidentDedupKey(), timeoutA2AExpiry, string(encodedMetadata)); err != nil {
...
func (p *a2aPublisher) checkDupAndLock(
	...
	v, err := p.db.Get(ctx, alert.IncidentDedupKey())
	if err != nil {
	...
	didLock, err := p.db.Lock(ctx, alert.IncidentDedupKey(), timeoutLock)
	if err != nil {
		return false, fmt.Errorf("failed to lock alert: %w", err)
	}
	if !didLock {
		// another instance is about to publish
		return true, ErrAlreadyPublishing
	}
...
func a2aResponseMetadata(response a2a.SendMessageResult) PublishMetadata {
	metadata := PublishMetadata{}
	if response == nil {
		return metadata
	}
